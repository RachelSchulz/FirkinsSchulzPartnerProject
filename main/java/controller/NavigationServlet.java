		...
		String act = request.getParameter("doThisToEmployee");
				...
				if(act.equals("delete")) {
					try {
						...
						ListEmployee employeeToDelete = dao.searchForItemById(tempId);
						dao.deleteItem(employeeToDelete);
					}
					catch (NumberFormatException e) {
						System.out.println("Forgot to select a name");
					}
				}
				else if(act.equals("edit")) {
					try{
						Integer tempId = Integer.parseInt(request.getParameter("id"));
						ListEmployee itemToEdit = dao.searchForItemById(tempId);
